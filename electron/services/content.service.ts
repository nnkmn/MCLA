    ...
    try {
      if (platform === ContentPlatform.CURSEFORGE) {
        ...
        if (isNaN(modId) || modId <= 0) {
          log.warn('Invalid CurseForge mod ID:', projectId)
          return []
        }
        ...
        if (!projectId || projectId.length < 3) {
          log.warn('Invalid Modrinth project ID:', projectId)
          return []
        }
  ...
  private curseForgeModToContentProject(mod: CurseForgeMod): ContentProject {
    ...
    return {
      ...
      loaders: mod.latestFiles.flatMap((f) =>
        f.sortableGameVersions
          .map((v) => v.name)
          .filter((name) => ['Forge', 'Fabric', 'Quilt', 'NeoForge'].includes(name))
      ),
