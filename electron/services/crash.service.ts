...
const CRASH_PATTERNS: CrashPattern[] = [
  ...
  },
  {
    ...
    causes: ['Java 堆内存设置不当', 'Mod 导致内存泄漏', '内存碎片化严重'],
    solutions: [
      '增加最大内存分配',
  ...
  },
  {
    ...
    causes: ['Mod 缺失依赖', 'Mod 版本不兼容', 'jar 文件损坏'],
    solutions: [
  ...
  },
  {
    ...
    causes: ['网络不稳定', '防火墙阻止连接', '服务器不可用'],
    solutions: ['检查网络连接', '关闭防火墙或添加例外', '尝试使用 VPN', '等待服务器恢复']
  },
  {
    // 文件系统
    ...
    causes: ['游戏目录权限不足', '文件路径包含特殊字符', '磁盘空间不足', '文件被其他程序占用'],
    solutions: [
      '以管理员身份运行启动器',
      '检查游戏目录权限',
      '确保路径不包含中文或特殊字符',
  ...
  },
  {
    ...
    causes: ['安装了重复的 Mod', 'Mod 包中包含重复文件', 'Mod 更新后 ID 变更'],
    solutions: ['检查并移除重复的 Mod', '解压 modpack 并检查', '更新所有 Mod 到最新版本']
  },
  {
    // Forge 初始化失败
    ...
    causes: ['Forge 版本与 Minecraft 版本不匹配', 'Forge 安装不完整', '与其他 Mod 冲突'],
    solutions: ['重新下载并安装对应版本的 Forge', '移除最近添加的 Mod', '清理并重新安装游戏']
  },
  {
    // Fabric 初始化失败
    ...
    causes: ['Fabric 版本与 Minecraft 版本不匹配', '缺少 Fabric API', '与其他 Mod 冲突'],
    solutions: [
...
export class CrashService {
   ...
   * 提取崩溃原因
   */
  private extractCrashCause(content: string): {
    exception: string
    message: string
    stackTrace: string[]
  } {
    ...
    if (stackTrace.length === 0) {
      const causedByMatch = content.match(
        /Caused by:\s*([\s\S]*?)(?=\n\s*(?:Caused by:|at |\n\n)|$)/i
      )
      if (causedByMatch) {
